...
export default function AdminPage({ profile }: { profile: Profile }) {
      ...
      <div className="min-h-screen bg-neutral-950 pb-10">
        <header className="bg-neutral-900 border-b border-neutral-800 px-4 py-3 flex items-center gap-3">
          <button
            ...
            className="w-9 h-9 rounded-lg bg-neutral-800 text-neutral-300 hover:bg-neutral-700 flex items-center justify-center shrink-0 text-lg">←</button>
        ...
        <div className="flex items-center gap-1 px-4 py-3 max-w-3xl mx-auto border-b border-neutral-800 overflow-x-auto">
          <button
            ...
            className={"px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap " +
              (clientViewTab === "entreno" ? "bg-white text-black" : "bg-neutral-800 text-neutral-400 hover:bg-neutral-700")}>
            🏋️ Entreno
          </button>
          <button
            onClick={() => {
            ...
            className={"px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap " +
              (clientViewTab === "checkin" ? "bg-white text-black" : "bg-neutral-800 text-neutral-400 hover:bg-neutral-700")}>
            📊 Control
          </button>
          <button
            onClick={() => setShowDietEditor(true)}
            className="px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap bg-neutral-800 text-neutral-400 hover:bg-neutral-700">
            🥗 Dieta
          </button>
                  ...
                  {(["all", "presencial", "auto"] as const).map(f => (
                    <button key={f} onClick={() => setCheckInFilter(f)}
                      className={"px-3 py-1.5 rounded-lg text-xs font-medium transition-colors " +
                        (checkInFilter === f ? "bg-white text-black" : "bg-neutral-800 text-neutral-400 hover:bg-neutral-700")}>
  ...
  return (
    <div className="min-h-screen bg-neutral-950 pb-10">
      <header className="bg-neutral-900 border-b border-neutral-800 px-4 py-4 flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-white">Panel Admin</h1>
          <p className="text-xs text-neutral-400">Hola, {profile.full_name}</p>
        </div>
        <button onClick={() => supabase.auth.signOut()} className="px-3 py-2 rounded-lg bg-neutral-800 text-neutral-300 text-sm hover:bg-neutral-700">
          Cerrar sesión
        </button>
      </header>
      ...
      <div className="flex gap-1 p-4 max-w-3xl mx-auto">
        {([["programas", "📋 Programas"], ["clientes", "👥 Clientes"], ["ejercicios", "🏋️ Ejercicios"]] as [Tab, string][]).map(([t, label]) => (
          <button key={t} onClick={() => setTab(t)}
            className={"px-4 py-2 rounded-lg text-sm font-medium transition-colors " + (tab === t ? "bg-white text-black" : "bg-neutral-800 text-neutral-300 hover:bg-neutral-700")}>
            {label}
          </button>
        ))}
      </div>
