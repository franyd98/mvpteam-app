-- ============================================================
-- 27_SQL_PLAN240_FRAN.sql  (v3 — CTE approach, type-safe)
-- Inserta el Plan 24.0 de Fran Villar para franvyother.
-- Ejecutar en Supabase → SQL Editor
-- ============================================================

-- Paso 1: Desactivar asignaciones previas
UPDATE diet_assignments SET active = false
WHERE client_id = '5818e102-0d55-412d-90cc-54cc92051799';

-- Paso 2: Insertar plan + comidas + opciones + asignación
WITH
...
c1_on AS (
  ...
  SELECT id, 'Comida 1 — Desayuno', '🌅', 'on', 0
  FROM new_plan
  RETURNING id
),
...
c1_off AS (
  INSERT INTO diet_meals (plan_id, name, emoji, day_type, sort_order)
  SELECT id, 'Comida 1 — Desayuno', '🌅', 'off', 1
  FROM new_plan
  RETURNING id
),
...
c1_off_a AS (
  INSERT INTO diet_options (meal_id, name, content, sort_order)
  SELECT id, 'Elaborada — Tortitas / Bizcocho', '[{"label": "Base", "isChoice": false, "items": ["125ml Claras de huevo + 1 Huevo entero"], "note": "Para tortitas: bátelo todo y cocina en sartén. Para bizcocho: añade levadura, 6-10 min en micro."}, {"label": "Hidratos a elegir", "isChoice": true, "items": [{"ingId": "harina_avena", "grams": 60}, {"ingId": "avena_copos", "grams": 60}, "80g Pan de Centeno o Espelta", {"ingId": "pan_tostado", "grams": 60}]}, {"label": "Grasa a elegir", "isChoice": true, "items": [{"ingId": "aceite_coco", "grams": 5}, {"ingId": "chocolate85", "grams": 10}, {"ingId": "aceite_oliva", "grams": 5}]}, {"label": "Fruta", "isChoice": false, "items": ["100-150g Fruta variada (excepto plátano)"]}]'::jsonb, 0
  FROM c1_off
  RETURNING id
),
...
c1_off_c AS (
  INSERT INTO diet_options (meal_id, name, content, sort_order)
  SELECT id, 'Opción 2 — Pan + Proteína fría', '[{"label": "Hidratos a elegir", "isChoice": true, "items": ["80g Pan de Centeno o Espelta (molde o normal)", {"ingId": "pan_tostado", "grams": 70}, "50g Tortas de arroz o maíz"]}, {"label": "Base", "isChoice": false, "items": [{"ingId": "huevo", "grams": 60}]}, {"label": "Proteína a elegir", "isChoice": true, "items": [{"ingId": "lomo_embuchado", "grams": 35}, "100g Queso fresco desnatado + 30g Fiambre pechuga pavo", {"ingId": "qso_fresco", "grams": 150}, {"ingId": "jamon", "grams": 60}, {"ingId": "atun_lata", "grams": 80}]}, {"label": "Grasa a elegir", "isChoice": true, "items": [{"ingId": "aceite_coco", "grams": 5}, {"ingId": "chocolate85", "grams": 10}, {"ingId": "aceite_oliva", "grams": 5}]}, {"label": "Fruta", "isChoice": false, "items": ["50g Arándanos congelados  ó  100g Frutos rojos  ó  100g Fresas o Frambuesas"]}]'::jsonb, 2
  FROM c1_off
  RETURNING id
),
...
c2_on AS (
  INSERT INTO diet_meals (plan_id, name, emoji, day_type, sort_order)
  SELECT id, 'Comida 2 — Almuerzo', '☕', 'on', 2
  FROM new_plan
  RETURNING id
),
...
c2_off AS (
  INSERT INTO diet_meals (plan_id, name, emoji, day_type, sort_order)
  SELECT id, 'Comida 2 — Almuerzo', '☕', 'off', 3
  FROM new_plan
  RETURNING id
),
...
c2_off_a AS (
  INSERT INTO diet_options (meal_id, name, content, sort_order)
  SELECT id, 'Pan + Proteína fría', '[{"label": "Hidratos a elegir", "isChoice": true, "items": ["80g Pan integral  ó  70g Pan tostado 100% integral", {"ingId": "tortas_arroz", "grams": 50}, {"ingId": "tortas_maiz", "grams": 50}]}, {"label": "Proteína a elegir", "isChoice": true, "items": [{"ingId": "lomo_embuchado", "grams": 50}, "100g Queso fresco desnatado + 60g Fiambre pechuga pavo", {"ingId": "qso_fresco", "grams": 250}, {"ingId": "jamon", "grams": 70}, {"ingId": "atun_lata", "grams": 90}]}, {"label": "Grasa", "isChoice": false, "items": [{"ingId": "aceite_oliva", "grams": 5}], "note": "Si comes pan añade tomate rallado para que no quede seco."}]'::jsonb, 0
  FROM c2_off
  RETURNING id
),
...
c3_on AS (
  INSERT INTO diet_meals (plan_id, name, emoji, day_type, sort_order)
  SELECT id, 'Comida 3 — Comida principal', '🍽️', 'on', 4
  FROM new_plan
  RETURNING id
),
...
c3_off AS (
  INSERT INTO diet_meals (plan_id, name, emoji, day_type, sort_order)
  SELECT id, 'Comida 3 — Comida principal', '🍽️', 'off', 5
  FROM new_plan
  RETURNING id
),
...
c4_on AS (
  INSERT INTO diet_meals (plan_id, name, emoji, day_type, sort_order)
  SELECT id, 'Comida 4 — Merienda', '🫐', 'on', 6
  FROM new_plan
  RETURNING id
),
...
c4_off AS (
  INSERT INTO diet_meals (plan_id, name, emoji, day_type, sort_order)
  SELECT id, 'Comida 4 — Merienda', '🫐', 'off', 7
  FROM new_plan
  RETURNING id
),
...
new_assignment AS (
  ...
  SELECT '5818e102-0d55-412d-90cc-54cc92051799', id, true, 'trainer'
  FROM new_plan
  RETURNING id
)

SELECT 'Plan 24.0 asignado correctamente a franvyother' AS resultado
FROM new_assignment;